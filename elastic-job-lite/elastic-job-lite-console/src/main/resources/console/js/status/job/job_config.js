...
function getJobParams() {
    var jobName = $("#job-overviews-name").text().replace("/","_");
    var jobParams;
    $.ajax({
        url: ctxPath+"/api/jobs/config/" + jobName,
        async: false,
...
function bindSubmitJobSettingsForm() {
    $("#update-job-info-btn").on("click", function(){
        ...
        if (bootstrapValidator.isValid()) {
            ...
            var jobRuntimeProps = $("#job-runtime-props").val();
            var description = $("#description").val();
            var reconcileIntervalMinutes = $("#reconcile-interval-minutes").val();
            var postJson = {jobName: jobName, jobType : jobType, jobClass : jobClass, shardingTotalCount: shardingTotalCount, jobParameter: jobParameter, cron: cron, streamingProcess: streamingProcess, maxTimeDiffSeconds: maxTimeDiffSeconds, monitorPort: monitorPort, monitorExecution: monitorExecution, failover: failover, misfire: misfire, shardingItemParameters: shardingItemParameters, jobShardingStrategyClass: jobShardingStrategyClass, jobProperties: {"executor_service_handler": executorServiceHandler, "job_exception_handler": jobExceptionHandler, "job_runtime_props": jobRuntimeProps}, description: description, scriptCommandLine: scriptCommandLine, reconcileIntervalMinutes:reconcileIntervalMinutes};
