...
function renderShardingTable() {
    var jobName = $("#job-name").text().replace("/","_");
    $("#sharding").bootstrapTable({
        url: ctxPath+"/api/jobs/" + jobName + "/sharding",
        cache: false,
        search: true,
        showRefresh: true,
        showColumns: true
...
function generateOperationButtons(val, row) {
    ...
    var disableButton = "<button operation='disable-sharding' class='btn-xs btn-warning'  item='" + row.item + "' data-lang='operation-disable'></button>";
    var enableButton = "<button operation='enable-sharding' class='btn-xs btn-success'  item='" + row.item + "' data-lang='operation-enable'></button>";
...
function bindDisableButton() {
    ...
    $(document).on("click", "button[operation='disable-sharding']", function(event) {
        var jobName = $("#index-job-name").text().replace("/","_");
        var item = $(event.currentTarget).attr("item");
        $.ajax({
        	cache: false,
            url: ctxPath+"/api/jobs/" + jobName + "/sharding/" + item + "/disable",
            type: "POST",
            success: function() {
                showSuccessDialog();
