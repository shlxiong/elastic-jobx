...
public final class InstanceService {
    ...
    public InstanceService(final CoordinatorRegistryCenter regCenter, final String jobName) {
        ...
        serverService = //new ServerService(regCenter, jobName);
        		JobServiceRegistry.getInstance().getServerService(jobName);
    ...
    //TODO("XIONGSL")
    public List<String> getAvailableJobInstanceIds(){
    	List<String> instanceIds = new ArrayList<String>();
    	for (JobInstance each : this.getAvailableJobInstances()) {
    		instanceIds.add(each.getJobInstanceId());
    	}
    	return instanceIds;
    }
     ...
     * 获取某个实例的Shardings
     * @param instanceId 实例
     * @param enabled 失效/有效的Shard，为空则返回所有
     */
    public List<Integer> getShardingItems(String instanceId, Boolean enabled){
    	List<Integer> list = this.getShardingItems(instanceId);
    	if (enabled == null) {
    		return list;
    	}
    	
    	List<Integer> shardingItems = new ArrayList<Integer>();
    	for (Integer item : list) {
    		String path = ShardingNode.getDisabledNode(item);  //sharding/%s/disabled
    		boolean flag = jobNodeStorage.isJobNodeExisted(path);
    		if (flag ^ enabled.booleanValue()) { //Boolean.logicXor
    			shardingItems.add(item);
    		}
    	}
    ...
    /**
     * TODO("XIONGSL")
     * 取得作业关闭之前，运行在一个实例上的所有分片ID
     * @param instanceId 实例ID
     */
    private List<Integer> getShardingItems(String instanceId){
//        if (!serverService.isAvailableServer(jobInstance.getIp())) {
//            return Collections.emptyList();
//        }
    	List<Integer> result = new LinkedList<>();
        int shardings = JobServiceRegistry.getInstance().getConfigService(jobName)
        				.load(true).getTypeConfig().getCoreConfig().getShardingTotalCount();
        for (int i = 0; i < shardings; i++) {
        	String failoverNode = FailoverNode.getExecutionFailoverNode(i);
            if (instanceId.equals(jobNodeStorage.getJobNodeData(ShardingNode.getInstanceNode(i)))
            		&& !jobNodeStorage.isJobNodeExisted(failoverNode)) {
                result.add(i);
            }
        }
        return result;
