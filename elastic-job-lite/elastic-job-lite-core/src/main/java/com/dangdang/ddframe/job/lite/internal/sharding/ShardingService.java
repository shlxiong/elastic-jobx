...
public final class ShardingService {
    ...
    public ShardingService(final CoordinatorRegistryCenter regCenter, final String jobName) {
        ...
        leaderService = //new LeaderService(regCenter, jobName);
        		registry.getLeaderService(jobName);
        configService = //new ConfigurationService(regCenter, jobName);
        		registry.getConfigService(jobName);
    ...
    public void shardingIfNecessary() {
        ...
        if (!leaderService.isLeaderUntilBlock()) {  //非主节点
    ...
    private void resetShardingInfo(final int shardingTotalCount) {
    	//TODO("XIONGSL"): 应该清除所有的信息  instance,running,disabled,failover,misfire
    ...
    public List<Integer> getLocalShardingItems() {
        ...
        if (JobRegistry.getInstance().isShutdown(jobName) || !serverService.isAvailableServer(jobInstance.getIp())) {
            return Collections.emptyList();
        }
        return getShardingItems(jobInstance.getJobInstanceId());
    }
