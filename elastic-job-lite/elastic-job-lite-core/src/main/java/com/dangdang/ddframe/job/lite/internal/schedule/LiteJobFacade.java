...
public final class LiteJobFacade implements JobFacade {
    ...
    public LiteJobFacade(final CoordinatorRegistryCenter regCenter, final String jobName, final List<ElasticJobListener> elasticJobListeners, final JobEventBus jobEventBus) {
        ...
        executionContextService = registry.getExecutionContextService(jobName);// new ExecutionContextService(regCenter, jobName);
    ...
    @Override
    public void postJobStatusTraceEvent(final String taskId, final State state, String message) {
    	TaskContext taskContext = TaskContext.from(taskId);
    	//TODO("XIONGSL")
    	int pos = message.indexOf(TaskContext.DELIMITER);
    	String contextId = message.substring(0, pos);
    	message = message.substring(pos+3);
    	JobStatusTraceEvent statusTraceEvent = new JobStatusTraceEvent(taskContext.getMetaInfo().getJobName(), taskContext.getId(),
    	...
    	statusTraceEvent.setContextId(contextId);
    	jobEventBus.post(statusTraceEvent);
